...
from aes_utils import generate_aes_key, aes_encrypt, aes_decrypt  # Import AES utils
...
def upload_file():
    ...
    # Retrieve file data from request.json
    data = request.json
    ...
    encryption_method = "AES"  # Change to AES
    # Encrypt the file content using AES
    start_time = time.time()
    aes_key = generate_aes_key()  # Generate a random AES key
    encrypted_content = aes_encrypt(aes_key, file_content.encode('utf-8'))
    # End timing after encryption
    end_time = time.time()
    encryption_time = end_time - start_time  # Calculate the encryption duration
    print('time to encrypt',encryption_time)
    print('encrypted content is:',encrypted_content)
    # Convert `encrypted_content` to a hex string for JSON serialization
    encrypted_content_hex = encrypted_content.hex()
    # Send the encrypted data and other details to the backend
    try:
        response = requests.post('http://localhost:8000/api/upload_file', json={
            "email": email,
            "filename": filename,
            "content": encrypted_content_hex,  # Send as hex for easy JSON transmission
            "encryption_method": encryption_method,
            "key_size": key_size,
            "aes_key": aes_key.hex()  # Include the AES key as hex
        })

        if response.status_code == 200:
            return jsonify({"message": f"File uploaded successfully with AES encryption (key size: {key_size})"}), 200
        else:
            error_data = response.json()
